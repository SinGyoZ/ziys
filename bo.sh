...
# 输入
# ./buildone.sh 服务器名称 CPU核数 内存大小mb 硬盘大小g SSH端口 外网起端口0 外网止端口0 下载速度mb 上传速度mb 是否启用IPV6(Y or N) 系统(留空则为debian11)
...
# IO
incus config device set "$name" root limits.read 5000MB
incus config device set "$name" root limits.write 5000MB
incus config device set "$name" root limits.read 50000iops
incus config device set "$name" root limits.write 50000iops
# cpu
incus config set "$name" limits.cpu.priority 0
incus config set "$name" limits.cpu.allowance 100%
incus config set "$name" limits.cpu.allowance 100ms/100ms
# 内存
